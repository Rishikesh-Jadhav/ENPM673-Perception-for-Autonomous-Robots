...
# Given K1 and K2 
K1 = np.array([[1758.23, 0, 829.15],
              [0, 1758.23, 552.78],
              [0, 0, 1]])
K2 = np.array([[1758.23, 0, 829.15],
              [0, 1758.23, 552.78],
              [0, 0, 1]])
...
def GetEpiLines(set1, set2, F, img1, img2, rectified = False):
    ...
    cv2.imwrite("chess_epipolar_lines.png", concat)
    cv2.waitKey()
...
pts3D4 = TriangulatePoints(K1, K2, matched_pairs_inliers, R, C)
...
count_thresh = int(pts3D4[0].shape[1] / 2)
...
# Creating 2D arrays for the left and right images
block_left_array_2D = []
...
plt.savefig('chess_depth_image.png')
plt.show()
